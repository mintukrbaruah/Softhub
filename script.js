...
async function loadFiles() { status.textContent = "Loading files…"; try { const response = await fetch(APPS_SCRIPT_URL); if (!response.ok) throw new Error(`Apps Script returned HTTP ${response.status}`); const data = await response.json(); renderFiles(data.files || []); status.textContent = ""; } catch (error) { renderFiles([]); count.textContent = "Error"; status.textContent = `Could not load files: ${error.message}`; console.error(error); } }
